  ...
  <>
    ...
    {type !== 'selection' && (
      <ElTableColumn
        ...
        sortable={sortable}
        v-slots={{
          header: () => <div style={{ textAlign }}>{title}</div>,
          default: ({ row, $index }) => {
            ...
            if (type === TdType.text) {
              return <div style={{ textAlign }}>{row[prop]}</div>
            } else if (type === TdType.icon) {
              {
                /* 纯icon */
              }
              return (
                <div class={style.icon} style={{ textAlign }}>
                  {!!iconOption?.prop ? (
                    <div style={{ textAlign }}>
                      {/* 如果有多种情况 */}
                      <SvgIcon
                    ...
                    v-slots={{
                      reference: () => (
                        <div style={{ textAlign }}>
                          <SvgIcon
                            name={
                              stateOption.filter((i) => i.value === row.completeStatus)[0]?.name
                            }
                          />
                        </div>
                      )
