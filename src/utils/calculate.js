...
export function calculateCommonAngle(coordinates) {
  const a = coordinates.find(a => a.label == 'ANS')
  const b = coordinates.find(a => a.label == 'PNS')
  const c = coordinates.find(a => a.label == 'P')
  const d = coordinates.find(a => a.label == 'Or')
  // 计算UN向量
...
export function calculateThreePointsAngle(str, coordinates) {
  ...
  const a = coordinates.find(a => {
    if (arr[0] === 'Na') {
      return a.label == 'N'
    } else {
      return a.label == arr[0]
    }

  })
  const b = coordinates.find(a => a.label == arr[1])
  const c = coordinates.find(a => a.label == arr[2])
...
export function calculateFourPointsAngle(str, coordinates) {
  let a, b, c, d
  if (str == 'U1-SN') {
    a = coordinates.find(a => a.label == 'UI')
    b = coordinates.find(a => a.label == 'UI_1')
    c = coordinates.find(a => a.label == 'S')
    d = coordinates.find(a => a.label == 'N')
  }
  if (str == 'SN-MP') {
    a = coordinates.find(a => a.label == 'S')
    b = coordinates.find(a => a.label == 'N')
    c = coordinates.find(a => a.label == 'Go_1')
    d = coordinates.find(a => a.label == 'Me')
  }
  if (str == 'L1-MP') {
    a = coordinates.find(a => a.label == 'LI')
    b = coordinates.find(a => a.label == 'LI_1')
    c = coordinates.find(a => a.label == 'Go_1')
    d = coordinates.find(a => a.label == 'Me')
  }
  if (str == 'FH-MP') {
    a = coordinates.find(a => a.label == 'P')
    b = coordinates.find(a => a.label == 'Or')
    c = coordinates.find(a => a.label == 'Go_1')
    d = coordinates.find(a => a.label == 'Me')

  }
  if (str == 'ODI') {
    a = coordinates.find(a => a.label == 'A')
  ...
  if (str == 'APDIAngle1') {
    a = coordinates.find(a => a.label == 'N')
    b = coordinates.find(a => a.label == 'P')
    c = coordinates.find(a => a.label == 'P')
    d = coordinates.find(a => a.label == 'Or')
  }
  if (str == 'APDIAngle2') {
    a = coordinates.find(a => a.label == 'A')
    b = coordinates.find(a => a.label == 'B')
    c = coordinates.find(a => a.label == 'N')
    d = coordinates.find(a => a.label == 'P')
  }
  // 计算UN向量
  const vectorUN = { x: b.x - a.x, y: b.y - a.y };
...
calculateWits({ x: 1, y: 2 }, { x: -1, y: -2 }, { x: 2, y: 1 }, { x: 2, y: -1 }, { x: -2, y: -1 }, {x: -2, y: 1})
// 计算垂足距离
